...
describe "Guessing CLI" do
  ...
  context 'user inputs' do
    it "responds to 'exit'" do
      expect { exit_guessing_game }.to output(/Goodbye!/).to_stdout
    end

    it "responds to a correct guess" do
      allow(self).to receive(:random_number_generator).and_return(2)
      expect(self).to receive(:gets).and_return("2").once
      expect(self).to receive(:gets).and_return("exit").once
      expect { run_guessing_game }.to output(/You guessed the correct number!/).to_stdout
    end

    it "responds to an incorrect guess" do
      allow(self).to receive(:random_number_generator).and_return(5)
      expect(self).to receive(:gets).and_return("2").once
      expect(self).to receive(:gets).and_return("exit").once
